...
Da sich die Anforderungen während der Implementierung geändert haben, musste die Datenbank angepasst werden. Für den Login musste eine Datenbank erstellt werden und die vorhandene Datenbank musste erweitert bzw. verändert werden.
%TODO: Datenbankentwurf mit ausführlicher Erklärung
...
Die Besonderheit der Übersichtsseite ist die Datenbankabfragen, sie setzt sich aus mehreren Befehlen zusammen. Die Abfrage findet in der Datei sensordaten.php statt und sieht wie folgt aus:
\begin{lstlisting}[caption=Datenbankabfrage der Sensordaten,frame=single,numbers=left,language=SQL]
SELECT *
FROM(
SELECT Messwert, Timestamp, M . SEN_ID,
Sensorname FROM Sensorknoten SK
INNER JOIN Sensorknoten_Messwerte AS SM ON
(SK . KN_ID = SM . KN_ID)
...
\subsection{Statistik}\label{Statistik}
	Die Messdaten der jeweiligen Sensorknoten können für eine Visualisierung genutzt werden. Da die Daten mit jeweils einem eindeutigen Zeitstempel versehen werden, können diese für einen Graphen mit einer Zeitabhängigkeit genutzt werden. Die notwendigen Daten werden mit Hilfe des Joins der \fullref{fig:Kapitel4/uebersichtjoin.jpg} zusammengebaut.
	\begin{figure}[htp]
